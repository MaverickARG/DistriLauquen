...
export default function App() {
  ...
  const buscarRepuestos = async () => {
    setCargando(true);
    try {
      const res = await fetch(`http://localhost:3001/api/buscar?q=${encodeURIComponent(busqueda)}&limite=100`);
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', fontFamily: 'sans-serif', padding: '20px' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', color: '#fff' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 'bold' }}>🚗 DistriLauquen</h1>
            <p style={{ margin: '4px 0 0 0', fontSize: '14px', color: '#94a3b8' }}>Catálogo y Listas de Precios Unificadas</p>
          </div>
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          ...
          {!cargando && busqueda && resultados.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px', backgroundColor: '#fff', borderRadius: '10px', color: '#64748b' }}>
